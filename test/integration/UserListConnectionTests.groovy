...
class UserListConnectionTests extends GroovyTestCase {
    ...
    void testListConnection() {
			...
			user.save();
			def author = GDOCUser.findByUsername('gdocUser');
			def mylist = new UserList(name:'myGeneList', author:author);
			println mylist.author.username
			...
			assertEquals 3, UserList.findByAuthor(author).list_items.size()
