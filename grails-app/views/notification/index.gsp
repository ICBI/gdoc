...
<body>
	...
	<g:def var="queryDate" value="${new java.sql.Date(notification.value.item.taskId.substring(notification.value.item.taskId.indexOf('_') + 1).toLong())}" />
		<g:if test="${notification.value.status != 'Running'}">
			<div style="float: left;"><g:link>${notification.value.item.taskId.substring(0, notification.value.item.taskId.indexOf('_'))}</g:link> (<g:formatDate date="${queryDate}" format="h:mm M/dd/yyyy"/> ) </div>
